...
void ofxBulletBaseSoftShape::setClusterCollisionParams() {
...
//	_softBody->m_cfg.collisions = 0;
	...
	_softBody->m_cfg.kSS_SPLT_CL	= 0.3f; // soft vs soft impulse split
	...
	_softBody->m_materials[0]->m_flags |= btSoftBody::fMaterial::DebugDraw;
...
#define IDX(_x_,_y_,_z_) (((_x_*(J+1)+_y_)*(K+1))+_z_)
			...
			for (unsigned int j = 0; j < J; ++j)
			{
				for (unsigned int k = 0; k < K; ++k)
				{
					...
					if ( j==J-1 ) {
						...
						_softBody->appendFace( p5, p6, p4 );
					}
					if ( j==0 )	{
						_softBody->appendFace( p2, p1, p3 );
						_softBody->appendFace( p3, p1, p0 );
					}
					if ( k==K-1 ){
						_softBody->appendFace( p2, p6, p1 );
						_softBody->appendFace( p6, p5, p1 );
					}
					if ( k==0 )	{
						_softBody->appendFace( p7, p3, p4 );
						_softBody->appendFace( p3, p0, p4 );
					}
					
					if ( i==0 )	{
						_softBody->appendFace( p1, p5, p4 );
						_softBody->appendFace( p0, p1, p4 );
					}
					if ( i==I-1 ) {
						_softBody->appendFace( p3, p7, p6 );
						_softBody->appendFace( p2, p3, p6 );
					}
	...
	_bCreated = true;
	_softBody->m_materials[0]->m_kLST	=	0.5f;
...
void ofxBulletBaseSoftShape::createFromPatch( btSoftRigidDynamicsWorld* a_world, ofVec3f topleft, ofVec3f topright, ofVec3f bottomleft, ofVec3f bottomright, float mass )
{
	_world = a_world;
