...
//--------------------------------------------------------------
void ofxBulletWorldSoft::update() {
...
//--------------------------------------------------------------
void ofxBulletWorldSoft::setCameraPosition( ofVec3f a_pos ) {
	_cameraPos = a_pos;
}

//--------------------------------------------------------------
void ofxBulletWorldSoft::setCamera( ofCamera* a_cam ) {
	_camera = a_cam;
}

//--------------------------------------------------------------
void ofxBulletWorldSoft::enableCollisionEvents() {
	bDispatchCollisionEvents = true;
}

//--------------------------------------------------------------
void ofxBulletWorldSoft::disableCollisionEvents() {
	bDispatchCollisionEvents = false;
}

// http://bulletphysics.org/mediawiki-1.5.8/index.php/Collision_Callbacks_and_Triggers
//--------------------------------------------------------------
void ofxBulletWorldSoft::checkCollisions() {
	//Assume world->stepSimulation or world->performDiscreteCollisionDetection has been called
	int numManifolds = world->getDispatcher()->getNumManifolds();
	//cout << "numManifolds: " << numManifolds << endl;
	for (int i = 0; i < numManifolds; i++) {
		btPersistentManifold* contactManifold =  world->getDispatcher()->getManifoldByIndexInternal(i);
#warning Casting (const btCollisionObject*) to (btCollisionObject*) -- not necessarily safe
...
//--------------------------------------------------------------
void ofxBulletWorldSoft::enableMousePickingEvents( short int a_filterMask ) {
	_mouseFilterMask = a_filterMask;
	bDispatchPickingEvents	= true;
}

//--------------------------------------------------------------
void ofxBulletWorldSoft::disableMousePickingEvents() {
...
//--------------------------------------------------------------
void ofxBulletWorldSoft::enableGrabbing(short int a_filterMask) {
	_mouseFilterMask = a_filterMask;
	bDispatchPickingEvents = true;
...
//--------------------------------------------------------------
void ofxBulletWorldSoft::disableGrabbing() {
	bRegisterGrabbing = false;
}


//--------------------------------------------------------------
void ofxBulletWorldSoft::enableDebugDraw() {
	if(!bHasDebugDrawer) {
...
//--------------------------------------------------------------
void ofxBulletWorldSoft::drawDebug() {
	if(!bHasDebugDrawer) {
		enableDebugDraw();
	}
...
//--------------------------------------------------------------
bool ofxBulletWorldSoft::checkWorld() {
	if(world == NULL) {
...
//--------------------------------------------------------------
void ofxBulletWorldSoft::removeMouseConstraint() {
	if ( _pickConstraint != NULL && world != NULL ) {
		//cout << "ofxBulletWorldSoft :: checkMousePicking : removing a mouse constraint" << endl;
...
//--------------------------------------------------------------
void ofxBulletWorldSoft::destroy() {
	
	cout << "ofxBulletWorldSoft :: destroy : destroy() " << endl;
	...
	if(world != NULL) {
		...
		cout << "ofxBulletWorldSoft :: destroy : num constraints= " << world->getNumConstraints() << endl;
	...
	if(world != NULL) {
		cout << "ofxBulletWorldSoft :: destroy : num collision objects= " << world->getNumCollisionObjects() << endl;
...
//--------------------------------------------------------------
void ofxBulletWorldSoft::mouseMoved( ofMouseEventArgs &a ) {
...
//--------------------------------------------------------------
void ofxBulletWorldSoft::mousePressed( ofMouseEventArgs &a ) {
	_bMouseDown = true;
	//cout << "ofxBulletWorldSoft :: mousePressed : x = " << a.x << " y = " << a.y << endl;
